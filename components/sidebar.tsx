...
export function Sidebar() {
      ...
      try {
        if (user?.email) {
          const response = await fetch(
            `https://script.google.com/macros/s/AKfycbzUhN6VdsCRpKoboczx4qxD1Q6TqiGRo-EaqTzrM7THF0Jd1DGYR3wBHqRjLn0IXCWE/exec?sheetId=1mXw76hqsqPZG3hBrcuujOOQe2CUp8uEoe7_6cM4nbvQ&email=${user.email}`
          )
  ...
  // FIXED: Updated click outside handler to exclude link clicks
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Don't close if clicking on a link inside the sidebar
  ...
  // FIXED: Added handler for link clicks
  const handleLinkClick = (e: React.MouseEvent, path: string) => {
    // Allow the link to navigate
    // Close menu after a tiny delay to ensure navigation happens
    setTimeout(() => {
      setIsMobileMenuOpen(false)
    }, 100)
  }
