...
export default function AddDocument() {
  ...
  const fetchMasterData = async () => {
    try {
      ...
      const dirs = result.data
        ...
        .filter((dir: string) => dir); // Remove empty values

      setDirectors(Array.from(new Set(dirs)));
  ...
  const uploadFileToGoogleDrive = async (file: File): Promise<string> => {
    ...
    try {
      ...
      formData.append("folderId", "1ki-5E8dqLesWtH9oX9pzd8kMwaEVQyJV");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of multipleFiles) {
        ...
        console.log(
          `Generated serial number: ${serialNumber} for document: ${file.name}`
        );
            ...
            <CardContent className="space-y-6 p-4 md:p-6 bg-gray-50">
              {multipleFiles.map((fileItem, index) => (
                ...
                >
                  ...
                  {fileItem.documentType === "Director" ? (
                    <div className="space-y-2 mb-3 md:mb-4">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleMultipleInputChange(index, "entityName", value)
                        }
                        required
                      >
                        <SelectTrigger className="border-gray-300 text-sm bg-white">
